...
- Add `type` on step -> map script phases (intro/example/activity/summary/transition).
- Remove `anchor` from editable model -> obsolete in chat UI.
...
- Treat validation errors as 400, missing entities as 404, conflicts as 409.
...
- Keep `scene 0` empty in backend steps -> avatar selection remains frontend flow.
